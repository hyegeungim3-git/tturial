...
export function designSummary(original,shown=original,pending=false) {
  ...
  if(!s)return `<div class="knit-summary invalid"><strong>설계 조건을 확인해 주세요</strong><p>${plan.errors.join('<br>')}</p></div>`;
  ...
  ${s.continuation?`<p class="measure-linked">완료 ${s.continuation.lockedRows}단 보존 · 실측 ${s.continuation.measurement.cm}cm 반영 · 금색선은 공통 보존 경계</p>`:''}<div class="knit-links"><button data-action="knit-edit">길이·소맷단 조정</button><button data-action="knit-details">계산 근거와 단별 도안 →</button></div></section>`;
}
export function designDetails(p) {
  const plan=compileDesign(p),s=plan.sleeve;if(!s)return plan.errors.join('<br>');
